...
{    
	...
	vNet_SetAddress(network_address3, 4);			// Set address on media 2
  ...
  if (vNet_DataAvailable())
  {
	...
	if(millis()-time>10000)
	{
		...
		{		
			...
			if(!vNet_Send(network_address2, msg1, 5, 0x10))
				Serial.println("Message 3 not sent");
		}
		else
		{	
			if(!vNet_Send(network_address1, msg2, 6, 0x20))
