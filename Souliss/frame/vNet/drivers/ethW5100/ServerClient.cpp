...
{
	if(W5100.readSnSR(usedsocket) == SnSR::CLOSED)
	{
...
{
	uint8_t s = W5100.readSnSR(usedsocket);
  
	return !(s == SnSR::LISTEN || s == SnSR::CLOSED || s == SnSR::FIN_WAIT || 
		(s == SnSR::CLOSE_WAIT && !W5100.getRXReceivedSize(usedsocket)));
}
...
uint8_t srvcln_dataavailable(uint16_t port) 
{
	...
	if((s == SnSR::ESTABLISHED || s == SnSR::CLOSE_WAIT))
		return W5100.getRXReceivedSize(usedsocket);
...
uint8_t srvcln_retrieve(uint8_t *data, uint8_t len) 
{
	...
	if(len>0 && len <= W5100.getRXReceivedSize(usedsocket)+1)
...
void srvcln_stop() 
{
	...
	while((W5100.readSnSR(usedsocket) != SnSR::CLOSED ) && (millis() - start_time < ETH_WAIT))
	    delay(1);
