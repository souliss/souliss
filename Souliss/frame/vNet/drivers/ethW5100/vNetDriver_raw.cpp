...
#include "vNetDriver_raw.h"
	...
	// Include the W5100 drivers only if not yet done by Media1
	#include "src/w5100.cpp"
#endif

uint8_t src_mac_addr[6];							// Source MAC address
uint8_t ethraw_frame[ETH_FRAME_LEN];				// Buffer for incoming data from W5100
...
void vNet_SetAddress_M3(uint16_t addr)
{
	// Set the MAC Address	
		...
		W5100.setMACAddress(src_mac_addr);
	#else
...
{	
	...
	*(ethstr.data+ETH_VNET_PAYLOAD) = len+1;
	...
	// Send data
	W5100.send_data_processing(RAW_SOCK, ethstr.data, ethstr.datalen);
    W5100.execCmdSn(RAW_SOCK, Sock_SEND_MAC);	
...
uint8_t vNet_DataAvailable_M3()
{
	...
	ethstr.datalen = W5100.getRXReceivedSize(RAW_SOCK);
	
	// If there is enough room in the buffer
	if(ethstr.datalen < ETH_FRAME_LEN)
	{
		W5100.recv_data_processing(RAW_SOCK, ethstr.data, ethstr.datalen);
		W5100.execCmdSn(RAW_SOCK, Sock_RECV);
	}
	else	// Discard the data
	{
		W5100.recv_data_processing(RAW_SOCK, ethstr.data, ETH_FRAME_LEN);
		W5100.execCmdSn(RAW_SOCK, Sock_RECV);
