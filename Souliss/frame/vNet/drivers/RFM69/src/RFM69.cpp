...
{
	oFrame* frame=0;	// data output frame
	uint8_t datalen=0;
	...
	// If the frame length is zero, is an oFrame	
	if(bufferSize)
		datalen = bufferSize;
	else if(bufferSize==0)	
	{
		frame = (oFrame*)buffer;				// Pointer to output frame
		oFrame_Define(frame);				// Set the frame
		datalen = oFrame_GetLenght();			// Get the total length	
	}
	
	// If the frame is to big, discard it
