...
U8 Souliss_RemoteDigIn(U8 pin, U8 value, U16 addr, U8 slot, bool filteractive=false)
{
	if(Souliss_DigIn(pin, value, 0, slot, filteractive) == value)
		return MaCaco_send(addr, MaCaco_FORCEREGSTR, 0x00, MaCaco_IN_s + slot, 1, value);
	else
		return 0;
}
...
{
	U8 value = Souliss_DigIn2State(pin, value_state_on, value_state_off, 0, slot);
...
{
	U8 value = Souliss_AnalogIn2Buttons(pin, value_button1, value_button2, 0, slot);
...
{
	U8 value = Souliss_LowDigIn2State(pin, value_state_on, value_state_off, 0, slot);
...
{
	U8 value = Souliss_DigInHold(pin, value, value_hold, 0, slot, holdtime)
...
*/	
/**************************************************************************/
U8 Souliss_RemoteLowDigInHold(U8 pin, U8 value, U8 value_hold, UU16 addr, U8 slot, U16 holdtime=1500)
{
	U8 value = Souliss_LowDigInHold(pin, value, value_hold, 0, slot, holdtime)
