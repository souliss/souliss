/* vNet Example 5, Data Transmission
  ...
  A standard Node send data to a device out of him subnet. 
  Data are sent over the SuperNode.
...
void setup()
{
  ...
  vNet_SetAddress(0x6601, 2);
  vNet_SetSubnetMask(0xFF00, 2);  
  
  // Assign manually a routing path that could not be determined
  // from the configuration, in this case all messages to subnet
  // 0x0000 are routed to 0x6511.
  vNet_SetRoutingTable(0x6511, 0x0000, 0);
